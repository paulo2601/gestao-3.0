  ...
  return <Dialog open={open} title="Lançar produção" description="Selecione o serviço executado e divida entre um ou mais colaboradores." onClose={close} onBack={close} onConfirm={()=>void submit()} confirmLabel="Salvar produção" loading={busy}>
    <div className="engineering-production-entry-form">
      ...
      <div className="engineering-production-entry-form__grid">
        ...
        {usesApartmentUnits?<><Select label="Pavimento" value={floorId} onChange={event=>{setFloorId(event.target.value);setUnitIds([]);}} options={[{value:'',label:'Selecione…'},...floors.map(item=>({value:item.id,label:item.name}))]} required/><div className="engineering-production-entry-form__units"><span>Apartamentos / Unidades</span>{floorId&&units.length===0?<span className="ui-muted">Nenhuma unidade cadastrada neste pavimento.</span>:units.map(item=><label key={item.id}><input type="checkbox" checked={unitIds.includes(item.id)} onChange={event=>setUnitIds(current=>event.target.checked?[...current,item.id]:current.filter(id=>id!==item.id))}/><span>{item.name}</span></label>)}</div></>:<Input label="Quantidade" type="number" value={executedQuantity} onChange={event=>setExecutedQuantity(event.target.value)} required/>}
