...
type HrDocumentDbRow={id:string;tenant_id:string;company_id:string;employment_contract_id:string;document_type:string;issued_on:string|null;notes:string|null;file_path:string|null;file_name:string|null;mime_type:string|null;file_size_bytes:number|null;created_at:string};
export type HrEmployeeDocument={id:string;tenantId:string;companyId:string;employmentContractId:string;documentType:string;issuedOn:string|null;notes:string|null;filePath:string|null;fileName:string|null;mimeType:string|null;fileSizeBytes:number|null;createdAt:string};
export async function listEmployeeDocuments(scopes:readonly {tenantId:string;companyId:string}[]):Promise<HrEmployeeDocument[]>{if(!scopes.length)return[];const client=getSupabaseClient();const groups=await Promise.all(scopes.map(async scope=>{const result=await client.from('employee_documents').select('id,tenant_id,company_id,employment_contract_id,document_type,issued_on,notes,file_path,file_name,mime_type,file_size_bytes,created_at').eq('tenant_id',scope.tenantId).eq('company_id',scope.companyId).not('file_path','is',null).order('created_at',{ascending:false}).returns<HrDocumentDbRow[]>();if(result.error)throw result.error;return result.data??[];}));return groups.flat().map(r=>({id:r.id,tenantId:r.tenant_id,companyId:r.company_id,employmentContractId:r.employment_contract_id,documentType:r.document_type,issuedOn:r.issued_on,notes:r.notes,filePath:r.file_path,fileName:r.file_name,mimeType:r.mime_type,fileSizeBytes:r.file_size_bytes,createdAt:r.created_at}));}
export async function uploadEmployeeDocument(input:{tenantId:string;companyId:string;employmentContractId:string;documentType:string;file:File;notes?:string|null}):Promise<void>{const client=getSupabaseClient();const safe=input.file.name.replace(/[^a-zA-Z0-9._-]+/g,'_');const path=`${input.tenantId}/${input.companyId}/${input.employmentContractId}/${crypto.randomUUID()}-${safe}`;const upload=await client.storage.from('hr-documents').upload(path,input.file,{...(input.file.type?{contentType:input.file.type}:{}),upsert:false});if(upload.error)throw upload.error;const row=await client.from('employee_documents').insert({tenant_id:input.tenantId,company_id:input.companyId,employment_contract_id:input.employmentContractId,document_type:input.documentType,issued_on:new Date().toISOString().slice(0,10),notes:input.notes?.trim()||null,file_path:path,file_name:input.file.name,mime_type:input.file.type||null,file_size_bytes:input.file.size});if(row.error){await client.storage.from('hr-documents').remove([path]);throw row.error;}}
export async function openEmployeeDocument(filePath:string):Promise<string>{const result=await getSupabaseClient().storage.from('hr-documents').createSignedUrl(filePath,60);if(result.error)throw result.error;return result.data.signedUrl;}
